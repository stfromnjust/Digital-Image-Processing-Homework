                             ...
                             int dw)
{
    ...
    for (theta = 0, pCountCur = pCount; theta < height; theta++)
    {
        for (rho = 0; rho < width - dw; rho++, pCountCur++)
        {
            if (*pCountCur + *(pCountCur + dw) > maxCount)
            {
                maxCount = *pCountCur + *(pCountCur + dw);
                *bstTheta = theta;
                *bstRho = rho - maxRho;
            }
        }
...
)
{
    ...
    // step.1-----初始化----------------------------------------//
    pBegin = pImg + y0 * width + x0; //轮廓起点的地址
    //不同链码对应的地址偏移量
    for (i = 0; i < 8; i++) offset[i] = dy[i] * width + dx[i];
    // step.2-----轮廓点的所有链码纵坐标变化量之和赋初值----------//
    for (sumC = 2; sumC <= 253; sumC++) //求sumC
    {
        if ((regionColor >= sumC - 2) && (regionColor <= sumC + 2)) continue;
        if ((nonColor >= sumC - 2) && (nonColor <= sumC + 2)) continue;
        break;
    }
    for (i = 0, pCur = pBegin; i < N; pCur += offset[pChainCode[i]], i++) *pCur = sumC;
    // step.3-----计算轮廓点的所有链码纵坐标变化量之和------------//
    inCode = pChainCode[N - 1]; //进入轮廓起点的链码
    for (i = 0, pCur = pBegin; i < N; i++)
    {
        outCode = pChainCode[i]; //从该轮廓点出发的链码
        *pCur += dy[inCode] + dy[outCode]; //像素的灰度值就是变化量之和
        inCode = outCode; //当前轮廓点的出发码就是下一个轮廓点的进入码
        pCur += offset[outCode]; //指向下一个轮廓点的地址
    }
    // step.4-----对填充起点和填充终点进行标记--------------------//
    for (i = 0, pCur = pBegin; i < N; pCur += offset[pChainCode[i]], i++)
    {
        sumDy = *pCur;
        if ((sumDy == sumC + 1) || (sumDy == sumC + 2)) *pCur = regionColor; //标记为填充起点
        else if ((sumDy == sumC - 1) || (sumDy == sumC - 2)) *pCur = nonColor; //标记为填充终点
    }
    // step.5-----按行在填充起点和填充终点之间进行填充------------//
    direction = isOuter ? 1 : -1; //外轮廓是从左向右,所以是+1;内轮廓反之
    for (i = 0, pCur = pBegin; i < N; pCur += offset[pChainCode[i]], i++)
    {
        if (*pCur == regionColor) //找到一个填充起点
        {
            pTst = pCur;
            while (*pTst != nonColor) //一直填充到终点
            {
                *pTst = regionColor;
                pTst += direction;
            }
            *pCur = nonColor; //该水平段已经填充过了,避免重复填充
        }
    }
    // step.6-----对轮廓点的颜色进行赋值-------------------------//
    for (i = 0, pCur = pBegin; i < N; pCur += offset[pChainCode[i]], i++) *pCur = contourColor;
}
